...
const CLIENT_ID =
  "170958026096-1delfs3g8tg4hoeg6bgs5ickhpe7k5pt.apps.googleusercontent.com"; // Given by Google when you set up your Google OAuth client ID: https://developers.google.com/identity/sign-in/web/sign-in
...
async function verify(CLIENT_ID, token) {
  ...
  try {
    const ticket = await client.verifyIdToken({
      // Function to inspect the user's "ticket" (token)
      idToken: token,
      audience: CLIENT_ID,
    });
...
async function TokenToUserID(user_session_token) {
  ...
  try {
    pool.query(
      "SELECT user_id FROM user_profile WHERE google_user_id = ?",
      google_user_id,
      function (error, result) {
        if (result == null) {
          ...
          console.log(
            "The result to be returned from calling this function: ",
            result[0].user_id
          );
...
const router = (app) => {
  ...
  app.post("/SignIn", async (request, response) => {
    ...
    if (!VerifiedTokenPayload) {
      ...
      try {
        ...
        pool.query(
          "SELECT * FROM user_profile WHERE google_user_id = ?",
          google_user_id,
          function (error, result) {
            ...
            if (result == null) {
              ...
              console.log(
                "No result from existing user query: Inserting new user into user_profile DB"
              );
              try {
                ...
                pool.query(
                  ...
                  (error, result) => {
                    console.log(
                      "Output of first user to be written: RESULT = ",
                      result,
                      "ERROR = ",
                      error
                    );
  ...
  app.post("/ProfileRoute", async (request, response) => {
    ...
    if (!VerifiedTokenPayload) {
      ...
      try {
        pool.query(
          "SELECT user_id, profile_picture FROM user_profile WHERE google_user_id = ?",
          google_user_id,
          (error, result) => {
  ...
  app.get("/Videos", (request, response) => {
    ...
    pool.query(
      "SELECT content_id, content, content_desc FROM user_content WHERE content_type = 'video' AND user_id = ? AND content_desc <= 5;",
      user_id,
      (error, result) => {
        ...
        try {
          ...
          console.log(
            "User content error (likely no data for this user)- VIDEOS"
          );
          response.send(false);
        }
      }
  ...
  app.get("/Podcasts", (request, response) => {
    ...
    pool.query(
      "SELECT content_id, content FROM user_content WHERE content_type = 'podcast' AND user_id = ? ",
      user_id,
      (error, result) => {
        ...
        if (result == null) {
          ...
          for (i = 0; i < RetrievedPodcastData.length; i++) {
            ...
            PodcastConcatString = PodcastConcatString.concat(
              contentId,
              "=",
              PodcastId,
              "&"
            ); //https://stackoverflow.com/a/36123716/6065710
  ...
  app.get("/PodcastAPIQuery", async (request, response) => {
    ...
    for (var content_id in PodcastAPIQueryId) {
      ...
      if (PodcastAPIQueryId.hasOwnProperty(content_id)) {
        ...
        var PodcastSearchIDAPI =
          "https://listen-api.listennotes.com/api/v2/podcasts/" +
          PodcastAPIQueryId[content_id];
        const response = await unirest
          .get(PodcastSearchIDAPI)
        ...
        RetrievedPodastData[content_id] = {
          title: PodcastTitle,
          image: PodcastImage,
  ...
  app.get("/PodcastEpisodes", (request, response) => {
    ...
    pool.query(
      "SELECT content_id, content, content_desc FROM user_content WHERE (content_type = 'podcast_episode' OR content_type ='podcast_episode_manual') AND user_id = ? ",
      user_id,
      (error, result) => {
        if (error) console.log("Content retrieval error:", error);
        ...
        if (result == null) {
          ...
          //console.log('No podcast data')
          response.send(false);
        } else if (result.content_desc === "podcast_episode_manual") {
          // Manually inserted podcasts
          console.log("To Do: write logic for user inserted podcast episodes.");
        } else {
          // Searched for podcasts
          ...
          for (i = 0; i < RetrievedPodcastEpisodeData.length; i++) {
            ...
            PodcastEpisodeConcatString = PodcastEpisodeConcatString.concat(
              contentId,
              "=",
              PodcastEpisodeId,
              "&"
            ); //https://stackoverflow.com/a/36123716/6065710
            PodcastEpisodeRedirectQuery =
              "/PodcastEpisodeAPIQuery" + PodcastEpisodeConcatString;
          }
  ...
  app.get("/PodcastEpisodeAPIQuery", async (request, response) => {
    ...
    for (var content_id in PodcastEpisodeAPIQueryId) {
      if (PodcastEpisodeAPIQueryId.hasOwnProperty(content_id)) {
        ...
        var PodcastEpisodeSearchIdAPI =
          "https://listen-api.listennotes.com/api/v2/episodes/" +
          PodcastEpisodeAPIQueryId[content_id];
        const response = await unirest
...
*/
  ...
  app.get("/DiscoverArticle", (request, response) => {
    ...
    // RETRIEVE USER CONTENT DATA
    pool.query(
      "SELECT content FROM user_content WHERE content_type = 'article' AND user_id = ? ",
      user_id,
      (error, result) => {
        if (error) console.log("Content retrieval error:", error);
        try {
